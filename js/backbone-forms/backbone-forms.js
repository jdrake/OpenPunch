...
//    monthNames: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
  ...
  editors.DateTime = editors.Base.extend({
    ...
    render: function() {
      ...
      //Create options
      var hoursOptions = _.map(_.range(1, 13), function(hour) {
      ...
      var ampmOptions = _.map(['AM','PM'], function(ampm) {
        return '<option value="'+ampm+'">' + pad(ampm) + '</option>';
      });
      ...
      this.$ampm = $el.find('[data-type="ampm"]');
    ...
    */
    getValue: function() {
      ...
      if (!date || !hour || !min || !ampm) return null;
...
//      console.log(date, hour, min, ampm);
      date.setHours(hour);
      date.setMinutes(min);
//      console.log(date, hour, min, ampm);
      return date;
    ...
    setValue: function(date) {
      ...
      this.$hour.val(setHour);
      this.$min.val(date.getMinutes());
      this.$ampm.val((hour < 12) ? 'AM' : 'PM');
